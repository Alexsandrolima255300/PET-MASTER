(function(){
  ...
  function loadCfg(){const b=get();cfg={x:Number(b.imageX)||0,y:Number(b.imageY)||0,scale:Number(b.imageScale)||1};}
  function apply(){loadCfg();const el=document.querySelector('.heroPets');if(!el)return;if(get().image){el.style.setProperty('background-size',`${100*cfg.scale}% auto`,'important');el.style.setProperty('background-position',`calc(50% + ${cfg.x}px) calc(50% + ${cfg.y}px)`,'important');}}
  function inject(){
    ...
    const stage=controls.querySelector('.bannerStage'), img=controls.querySelector('.stageImage'), range=controls.querySelector('.sizeRange'), out=controls.querySelector('.sizeValue');
    loadCfg(); range.value=cfg.scale; out.textContent=Math.round(cfg.scale*100)+'%';
    function sync(){const b=get();if(!b.image)return;img.src=b.image;img.style.transform=`translate(${cfg.x}px,${cfg.y}px) scale(${cfg.scale})`;apply();}
    ...
    if(save)save.addEventListener('click',()=>{const b=get();b.imageX=Math.round(cfg.x);b.imageY=Math.round(cfg.y);b.imageScale=Number(cfg.scale.toFixed(2));set(b);setTimeout(()=>location.reload(),150)});
  }
